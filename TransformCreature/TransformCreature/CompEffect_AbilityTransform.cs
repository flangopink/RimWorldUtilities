...
namespace TransformCreature
{
    public class CompEffect_AbilityTransform : CompAbilityEffect
    {
		...
		public override void Apply(LocalTargetInfo target, LocalTargetInfo dest)
		{
			...
			if (Props.makeSmoke) FleckMaker.ThrowSmoke(targetPawn.DrawPos, targetPawn.Map, 2f);
			if (Props.makeSparks) FleckMaker.ThrowMicroSparks(targetPawn.DrawPos, targetPawn.Map);
			if (Props.makeGlow) FleckMaker.ThrowHeatGlow(targetPawn.Position, targetPawn.Map, 2f);
